...
def __report(token: Token) -> None:
    ...
    # vera.report(token, "MINOR:C-L3")
    vera.report(vera.Token(token.value, token.column, token.line, "L3", token.file), "MINOR:C-L3")
...
def is_space_between_identifier_and_left_paren_valid(line_tokens: list[Token], i: int) -> Validity:
    return Validity.VALID if i + 2 >= len(line_tokens) or line_tokens[i + 2].name == 'star' else Validity.INVALID

# The following token pairs must never be separated by a space
FORBIDDEN_SPACE_NEIGHBORS: set[tuple[str, str, Optional[Callable[[list[Token], int], Validity]]]] = {
    ('identifier', 'leftparen', is_space_between_identifier_and_left_paren_valid),
    ('sizeof', 'leftparen', None),
    ('rightparen', 'semicolon', None),
}
...
def _is_unwanted_space(prev_token_name: str | None, token_name: str, next_token_name: str | None, i: int,
                       line_tokens: list[Token]) -> bool:
    if token_name != 'space':
        return False
    for (prev, nxt, check) in FORBIDDEN_SPACE_NEIGHBORS:
        if prev == prev_token_name and nxt == next_token_name:
            if check is None:
                return True
            return check(line_tokens, i) == Validity.INVALID
    return (
            prev_token_name in FORBIDDEN_SPACE_PREDECESSORS
...
def _check_unwanted_spaces(token, i, line_tokens):
    ...
    if (
            ...
            or _is_unwanted_space(previous_token_name, token.name, next_token_name, i, line_tokens)
    ):
        __report(token)
