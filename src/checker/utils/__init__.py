...
def replace_non_ascii_by_space(s: str) -> str:
    ascii_str = ''
    for c in s:
        if ord(c) < 128:
            ascii_str += c
        else:
            ascii_str += ' ' * len(c.encode('utf-8'))
    return ascii_str


def __remove_between(lines: List[str], token: Token, begin_token: str = "//", end_token: str | None = None) -> None:
    for offset, value in enumerate(token.value.split("\n")):
        ...
        if (len(line) - (len(head) + value_length)) > 0:
            tail = line[-(len(line) - (len(head) + value_length)):]
        else:
            tail = ""

        if begin_token and end_token and value.startswith(begin_token) and value.endswith(end_token):
            line = head + begin_token + ' ' * (value_length - (len(begin_token) + len(end_token))) + end_token + tail
        elif begin_token and value.startswith(begin_token):
            line = head + begin_token + ' ' * (value_length - len(begin_token)) + tail
        elif end_token and value.endswith(end_token):
            line = head + ' ' * (value_length - len(end_token)) + end_token + tail
        else:
            line = ' ' * len(line)
...
def get_lines(file: str, replace_comments=False, replace_stringlits=False) -> List[str]:
    lines = [replace_non_ascii_by_space(l) for l in vera.getAllLines(file)]
    if replace_comments:
